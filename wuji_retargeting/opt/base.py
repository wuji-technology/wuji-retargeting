...
class BaseOptimizer(ABC):
    ...
    def __init__(self, config: dict):
        ...
        # Load URDF
        urdf_path = str((_PACKAGE_ROOT / f"urdf/{self.hand_side}.urdf").resolve())
    ...
    def from_config(cls, config: dict) -> "BaseOptimizer":
        ...
        opt_type = config.get('optimizer', {}).get('type', 'AdaptiveOptimizerQP')

        if opt_type == 'AdaptiveOptimizerQP':
            return AdaptiveOptimizerQP(config)
        elif opt_type == 'AdaptiveOptimizerAnalytical':
            from .adaptive_analytical import AdaptiveOptimizerAnalytical
            return AdaptiveOptimizerAnalytical(config)
        elif opt_type == 'TipDirVecOptimizer':
            from .tip_dir_vec import TipDirVecOptimizer
            return TipDirVecOptimizer(config)
        elif opt_type == 'TipDirVecOptimizerQP':
            from .tip_dir_vec_qp import TipDirVecOptimizerQP
            return TipDirVecOptimizerQP(config)
        elif opt_type == 'TipDirVecOptimizerAnalytical':
            from .tip_dir_vec_analytical import TipDirVecOptimizerAnalytical
            return TipDirVecOptimizerAnalytical(config)
        else:
