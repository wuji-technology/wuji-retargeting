...
Wuji Hand 手部姿态重定向系统。基于自适应解析优化的高精度重定向，支持 Apple Vision Pro 手部追踪实时遥操作。
...
- [使用方法](#使用方法)
  - [环境要求](#环境要求)
    - [Apple Vision Pro 配置](#apple-vision-pro-配置)
  - [安装](#安装)
  - [运行](#运行)
    - [快速开始](#快速开始)
    - [命令参考](#命令参考)
...
| 目录 | 说明 |
|------|------|
| `wuji_retargeting/` | 核心包，包含重定向接口、优化器模块、运动学和坐标变换 |
| `wuji_retargeting/opt/` | 优化器实现，包括自适应解析优化器 |
| `wuji_retargeting/wuji_hand_description/` | Wuji Hand 的 URDF 和 mesh 子模块 |
| `example/` | 仿真和硬件控制的示例脚本 |
| `example/input_devices/` | 输入设备模块（Vision Pro、MediaPipe 回放） |
...
| `--play FILE` | - | 播放 MediaPipe 录制（`--input mediapipe_replay` 的快捷方式） |
...
**注意**：默认配置针对 Apple Vision Pro 调优。使用其他输入设备时，可能需要根据手型调整 `scaling` 和 `segment_scaling` 参数。
